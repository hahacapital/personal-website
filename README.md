...
- **Case studies** — `src/pages/zh/work/[slug].astro` and `src/pages/en/work/[slug].astro`. Dynamic routes whose `getStaticPaths()` maps over the `workZh` / `workEn` collections (one static page per entry) and passes each `entry.data` field plus the rendered `<Content />` body through the shared `src/layouts/CaseStudyLayout.astro`. All five flagship case studies (bilingual, 10 pages) render through this one template; it works against whatever is in the collection today (the `_fixture` placeholder) or the real content once it lands.
  ...
  - **Content** for each case study lives as Markdown in `src/content/work/zh/` and `src/content/work/en/`, validated by the `workSchema` in `src/content.config.ts`. The real five land in later tasks; a `_fixture.md` placeholder per language keeps the collections valid until then (ids beginning `_` are excluded from the Home `CaseIndex`).
